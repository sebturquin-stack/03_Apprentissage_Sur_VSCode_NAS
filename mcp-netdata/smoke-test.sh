...
check_mcp_to_netdata() {
    ...
    output="$(compose_cmd exec -T "$MCP_SERVICE" node -e "const url='http://netdata:19999/api/v1/info'; fetch(url).then(r=>{console.log('STATUS=' + r.status); process.exit(r.ok?0:1);}).catch(()=>process.exit(2));" 2>/dev/null || true)"
